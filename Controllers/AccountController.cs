    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.Pseudo, Email = model.Email ,PhoneNumber=model.Tel};
                ...
                if (result.Succeeded)
                {
                    ...
                    //on recupere le resultat du combobox pour determiner le type d'utilisateur
                    UserInstance userC = new UserInstance();
                    UserDal udal = new UserDal();
                    int num = Convert.ToInt32(Request.Form["type"]);
                    userC.UserName = Request.Form["Pseudo"];
                    int indice = rnd.Next(0, 20);
                    int indice2 = rnd.Next(21, 50);
                    //on genere aleatoirement 2 nombre qu'on combine avec les 2 premières lettres du type d'utilisateur pour former le code 
                    ...
                    if (num == 1)
                    {
                        ...
                        Client c = new Client();
                        c.code = "C" + indice2 + "L" + indice;
                        dal.add(c);

                        userC.UserType = "Client";
                        userC.Code = "C" + indice2 + "L" + indice;
                        udal.add(userC);
                    }
                    else if (num == 2)
                    {
                        ...
                        Agent a = new Agent();
                        a.code = "A" + indice2 + "G" + indice;
                        dal.create(a);

                        userC.UserType = "Agent";
                        userC.Code = "A" + indice2 + "G" + indice;
                        udal.add(userC);
                    }
                    else if (num == 3)
                    {
                        FournisseurDal dal = new FournisseurDal();
                        Fournisseur f = new Fournisseur();
                        f.code = "F" + indice2 + "O" + indice;
                        f.UserName= Request.Form["Pseudo"];
                        dal.add(f);

                        userC.UserType = "Fournisseur";
                        userC.Code = "F" + indice2 + "O" + indice;
                        udal.add(userC);

                    }
                    else
                    {
                        userC.UserType = "Admin";
                        userC.Code = "Admin";
                        udal.add(userC);
                    }
